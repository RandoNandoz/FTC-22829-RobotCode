...
@TeleOp(name = "MecanumDriveSeanLee")
public class MecanumDriveSeanLee extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        var slide = new FourStageViperSlide(63.447, -4000, new MotorEx(hardwareMap, "mSlide", Motor.GoBILDA.RPM_312), 1, telemetry);
...
//            telemetry.addData("Back Right Motor Encoder Ticks: ", String.format(Locale.CANADA, "%d ticks", mBackRight.getCurrentPosition()));
            ...
            if (operatorOp.getButton(GamepadKeys.Button.RIGHT_BUMPER)) {
                ...
                slide.autoReset();
            } else if (operatorOp.getButton(GamepadKeys.Button.Y)) {
                slide.goToJunction(JunctionType.MEDIUM);
            } else if (operatorOp.getButton(GamepadKeys.Button.X)) {
            ...
            if (operatorOp.getButton(GamepadKeys.Button.DPAD_DOWN)) {
                claw.open();
                slide.autoReset();
            }
